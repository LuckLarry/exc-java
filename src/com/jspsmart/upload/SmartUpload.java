...
public class SmartUpload {
	...
	public final void initialize(HttpServletRequest request, HttpServletResponse response)
			throws ServletException {
		m_application = request.getServletContext();
		m_request = request;
		m_response = response;
	}
	...
	public void upload() throws SmartUploadException, IOException,
		...
		do {
			...
			if (isFile) {
				...
				/**
				 * 原来的代码 String value = new String(m_binArray, m_startData,
				 * (m_endData - m_startData) + 1);
				 */
				/**
				 * 2010-4-17 修改 ，解决取得request的参数的中文编码问题
				 */
	...
	public int save(String destPathName) throws SmartUploadException,
	...
	public int save(String destPathName, int option)
		...
		for (int i = 0; i < m_files.getCount(); i++)
			if (!m_files.getFile(i).isMissing()) {
				m_files.getFile(i)
						.saveAs(destPathName + m_files.getFile(i).getFileName(),
								option);
				count++;
			}
	...
	public void downloadFile(String sourceFilePathName, String contentType,
			String destFileName) throws SmartUploadException, IOException,
			ServletException {
		downloadFile(sourceFilePathName, contentType, destFileName, 65000);
	}

	public void downloadFile(String sourceFilePathName, String contentType,
			String destFileName, int blockSize) throws SmartUploadException,
			IOException, ServletException {
		...
		if (destFileName == null)
			m_response.setHeader("Content-Disposition", "attachment; filename="
					.concat(String.valueOf(getFileName(sourceFilePathName))));
	...
	public void downloadField(ResultSet rs, String columnName,
			String contentType, String destFileName) throws SQLException,
			IOException, ServletException {
		if (rs == null)
			throw new IllegalArgumentException(
	...
	public void fieldToFile(ResultSet rs, String columnName,
			String destFilePathName) throws SQLException, SmartUploadException,
			IOException, ServletException {
		try {
